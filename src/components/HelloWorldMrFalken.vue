      ...
      methods:{
                ...
                if(command.length>0)
                    switch(command.trim().toLowerCase()){
                        case "linkedin":
                            ...
                            var self = this;
                            setTimeout(function() { window.open("https://www.linkedin.com/in/giacomogiallombardo/", '_blank'); },2000);
                            return true
                            break;
                        case "info":
                            ...
                            this.writeOnConsole("- C# , PowerShell",4000)
                            this.writeOnConsole("- C, C (openGL) / Aspx",4200)
            ...
            printBanner:function( mobile ){
                ...
                if(!mobile) {
                    ...
                    this.writeOnConsole("This is an a simple website where you can't use the mouse in order to navigate it. Type \"help\" ( without quotes ) top get some available commands...", 1800)
                    this.writeOnConsole(" ", 3500)
                    var self = this;
                    setTimeout(function () {
                        self.showinput = true
            ...
            writeOnConsole: function ( mytext , delay ) {
                ...
                if(mytext.length >=this.column){
                  ...
                  if(mytext.length>this.column){
                      if(mytext.substring(this.column-1).length>this.column) {
                          ...
                          this.writeOnConsole(mytext.substring(this.column-1, mytext.length),delay+this.calculateDelay(mytext.substring(this.column-1, mytext.length)))
                      }
                      else {
                          console.log("lunghezzaa",mytext.substring(this.column-1))
                          this.writeOnConsole(mytext.substring(this.column-1), delay+this.calculateDelay(mytext.substring(this.column-1)))
                      }
                  }
                }
                else{
